...
function launch_dill() {
    ...
    if [ "$os_type" == "Darwin" ]; then
        if [ "$chip" == "arm64" ]; then
            ...
            if [ ! -f "$dill_darwin_file" ]; then
                if [ "$download" != "0" ]; then
                    curl -O $DILL_DARWIN_ARM64_URL
                else
                    echo "$dill_darwin_file does not exist and download is disabled."
                    exit 1
                fi
            else
                echo "$dill_darwin_file already exists, skipping download."
            fi
        ...
        if [ "$chip" == "x86_64" ] && [ -f /etc/os-release ]; then
            ...
            if [ "$ID" == "ubuntu" ]; then
                ...
                if [ $major_version -ge 20 ]; then
                    echo "Supported, OS: $ID $VERSION_ID, chip: $chip"
                    echo ""
                    if [ ! -f "$dill_linux_file" ]; then
                        if [ "$download" != "0" ]; then
                            curl -O $DILL_LINUX_AMD64_URL
                        else
                            echo "$dill_linux_file does not exist and download is disabled."
                            exit 1
                        fi
                    else
                        echo "$dill_linux_file already exists, skipping download."
                    fi
